 ...
 * (before adding cuts) TODO: Adapt it to when we have cuts?
 * 
 */
...
public class TabuSearch {
	...
	int forbiden = 5;/// MUYYYYY sensible...  5 y 15 los he probado.
	...
	public void run(ArrayList<Integer>basisRoutes, double shift){
		...
		//System.out.println(basisRoutes.size());
		for (int i = 0; i < basisRoutes.size(); i++) { 
			genCopy();
			tsIterations = 0;
			resetTabuSearch();
			ArrayList<Integer> route = Master.getPaths().get(basisRoutes.get(i)).route;//AlgorithmHandler.pool.get(basisRoutes.get(i)); //basisRoutes.get(i)
	...
	private void generateNeighbors() {
		...
		if(operator == REMOVE ){//Borrar
			...
			upDateRoutes("TS Delete", 0);//Si lo quieto la R208 hacia las dos ramificaciones en 3k segudnos
			fillPool();
		}else if( operator == INSERT){ // insertar
	...
	private void fillPool() {
		...
		if (routesCost < Master.rcCriteriaHeuristics && routes.size()>2 && !metaheuristicHandler.routesPoolRC.containsKey(keyS)) {
			//System.out.println("tabu finds something: rc " + routesCost + "  " + keyS );
	...
	private void upDateRoutes(String origen, int varIndex) {
		...
		for (int i = 1; i < routes.size(); i++) {
			...
			if(customer.arrivalTime >customer.tw_b){
				System.out.println("TIEMPOOOOOOOO: " +  customer.arrivalTime);
				System.out.print("ERROR FATAL Tabu Search: columa infactible por TW_b: En " + origen + " Columna " + varIndex  + "  generador: " + Master.generator.get(varIndex));
				System.out.println("\nRuta TS " + routes);
				System.out.println("El cliente " +customer.id + "  esta jodido" );
			}
