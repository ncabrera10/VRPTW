...
public final class ColumnGenerationDirector extends ColGen<VRPTW, RoutePattern, PA_PricingProblem> {
	...
	public void solve(long timeLimit) throws TimeLimitExceededException{
		...
		// Initializes the number of paths:
		
		int numP = 0;
		
		//Identify whether the pricing problem generated new columns: 
		
		boolean foundNewColumns=false; 
		
		//Identify whether the master problem violates any valid inequalities:
		boolean hasNewCuts; 
		
		//Boolean variable to keep the loop going:
		...
		do{
			...
			// Stores the number of paths found:
			
			numP = newColumns.size();
			
			if(numP == 0) { // If the number paths found is zero, then we can update the bound:
